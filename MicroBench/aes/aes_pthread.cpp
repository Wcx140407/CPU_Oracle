...
typedef struct {
    ...
    int key_size;
    int input_size;
    std::vector<uint8_t*> plaintexts;  // 明文数据
    uint8_t* key;                     // 密钥
    long long start_time;             // 线程开始时间
    long long end_time;               // 线程结束时间
} ThreadData;
...
// 生成随机数据集
std::vector<uint8_t*> gen_random_data(int input_size, int block_size) {
    std::vector<uint8_t*> data;
    data.reserve(input_size);
    for (int i = 0; i < input_size; i++) {
        data.push_back(gen_random(block_size));
    }
    return data;
}

// 释放数据集内存
void free_data(std::vector<uint8_t*>& data) {
    for (auto ptr : data) {
        delete[] ptr;
    }
    data.clear();
...
void* thread_worker(void* arg) {
    ...
    if (data->key_size == 128) {
        key_size_bytes = AES_KEY_SIZE_128;
    } else if (data->key_size == 192) {
        key_size_bytes = AES_KEY_SIZE_192;
    } else if (data->key_size == 256) {
        key_size_bytes = AES_KEY_SIZE_256;
    }
    
    // 执行多轮测试
    for (int round = 0; round < data->rounds; round++) {
        // 初始化AES上下文
        AesInitialise(&context, data->key, key_size_bytes);
        
        // 处理所有输入数据
        for (size_t i = 0; i < data->plaintexts.size(); i++) {
            uint8_t enc_block[AES_BLOCK_SIZE];
            uint8_t dec_block[AES_BLOCK_SIZE];
            
            // 加密
            AesEncrypt(&context, data->plaintexts[i], enc_block);
            
            // 解密
            AesDecrypt(&context, enc_block, dec_block);
            
            // 可以在这里添加验证逻辑
            // if (memcmp(dec_block, data->plaintexts[i], AES_BLOCK_SIZE) != 0) {
            //     printf("Thread %d: Decryption failed!\n", data->thread_id);
            // }
        }
    }
    
    // 记录结束时间
    data->end_time = get_current_time_ms();
...
void test_aes_concurrent(int rounds, int num_threads, const Config* configs, int num_configs) {
    if (num_threads <= 0 || num_threads > MAX_THREADS) {
        printf("错误：线程数必须在1到%d之间\n", MAX_THREADS);
        return;
    }
    ...
    long long total_start = get_current_time_ms();
    
    // 为每个配置创建线程
    pthread_t threads[num_configs];
    ThreadData thread_data[num_configs];
    
    // 创建并启动线程
    for (int i = 0; i < num_configs; i++) {
        thread_data[i].thread_id = i;
        thread_data[i].rounds = rounds;
        thread_data[i].key_size = configs[i].key_size;
        thread_data[i].input_size = configs[i].input_size;
        
        // 生成密钥
        int key_size_bytes = 0;
        if (configs[i].key_size == 128) {
            key_size_bytes = 16;
        } else if (configs[i].key_size == 192) {
            key_size_bytes = 24;
        } else if (configs[i].key_size == 256) {
            key_size_bytes = 32;
        }
        thread_data[i].key = gen_random(key_size_bytes);
        
        // 生成测试数据
        thread_data[i].plaintexts = gen_random_data(configs[i].input_size, AES_BLOCK_SIZE);
    ...
    long long total_end = get_current_time_ms();
    
    // 输出结果
    printf("\n测试结果:\n");
    printf("配置号 | 密钥大小 | 数据量 | 线程ID | 开始时间(ms) | 结束时间(ms) | 耗时(ms)\n");
    printf("----------------------------------------------------------------------------\n");
    
    for (int i = 0; i < num_configs; i++) {
        long long duration = thread_data[i].end_time - thread_data[i].start_time;
        printf("%6d | %8d | %6d | %6d | %12lld | %12lld | %8lld\n",
               i, configs[i].key_size, configs[i].input_size,
               thread_data[i].thread_id, thread_data[i].start_time,
               thread_data[i].end_time, duration);
        
        // 清理资源
        delete[] thread_data[i].key;
        free_data(thread_data[i].plaintexts);
    }
...
int read_config_from_file(const char* filename, Config* configs, int max_configs) {
    ...
    // 读取配置
    while (fgets(line, sizeof(line), fp) != NULL && count < max_configs) {
        // 跳过空行
        ...
        // 解析配置行
        int key_size, input_size;
        if (sscanf(line, "%d,%d", &key_size, &input_size) == 2) {
            configs[count].key_size = key_size;
            configs[count].input_size = input_size;
            count++;
        }
    }
...
int main(int argc, char* argv[]) {
    // 参数解析
    if (argc < 5) {
        printf("用法: %s <测试轮数> <线程数> <配置文件> <运行模式>\n", argv[0]);
        printf("\n参数说明:\n");
        printf("  测试轮数: 每个配置执行的测试次数\n");
        printf("  线程数: 并行执行的线程数 (1-%d)\n", MAX_THREADS);
        printf("  配置文件: 包含测试配置的CSV文件\n");
        printf("  运行模式: 'seq'顺序执行 或 'con'并发执行\n");
        printf("\n配置文件格式 (CSV):\n");
        printf("  key_size,input_size\n");
        printf("  128,1000\n");
        printf("  192,2000\n");
        printf("  256,3000\n");
        return 1;
    ...
    // 读取配置
    Config configs[MAX_CONFIG_NUM];
    int num_configs = read_config_from_file(config_file, configs, MAX_CONFIG_NUM);
    ...
    if (strcmp(mode, "con") == 0) {
        // 并发模式
        test_aes_concurrent(rounds, num_threads, configs, num_configs);
    } else if (strcmp(mode, "seq") == 0) {
        // 顺序模式（兼容原程序）
        long long start_time = get_current_time_ms();
        printf("AES顺序测试开始: %lld\n", start_time);
        
        for (int i = 0; i < num_configs; i++) {
            long long config_start = get_current_time_ms();
            printf("配置 %dx%d 开始: %lld", 
                   configs[i].key_size, configs[i].input_size, config_start);
            
            // 这里可以调用原来的test_aes函数或类似逻辑
            test_aes_concurrent(rounds, 1, &configs[i], 1);
            
            long long config_end = get_current_time_ms();
            printf("配置 %dx%d 结束: %lld, 耗时: %lld ms\n",
                   configs[i].key_size, configs[i].input_size,
                   config_end, config_end - config_start);
        }
        
        long long end_time = get_current_time_ms();
        printf("AES顺序测试结束: %lld\n", end_time);
        printf("总运行时间: %lld ms\n", end_time - start_time);
    } else {
        printf("错误：运行模式必须是 'seq' 或 'con'\n");
        return 1;
    }
