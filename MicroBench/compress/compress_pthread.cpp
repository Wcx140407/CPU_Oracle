...
int rounds_per_file = 1;        // 每个文件的处理轮数
int total_files = 0;            // 总文件数

/* ===================== 函数声明 ===================== */
int  	main			ARGS((int,char **));
void  	Usage			ARGS((int));
void  	comprexx		ARGS((const char *, int));
...
void init_thread_local_data(ThreadLocalData *data)
{
    ...
    data->force = 1;           // 并行模式下强制覆盖
    data->quiet = 1;           // 并行模式下静默
    data->maxbits = BITS;
    data->exit_code = -1;
}
...
{
#ifdef MAXSEG_64K
    memset(data->htab0, -1, sizeof(data->htab0));
    memset(data->htab1, -1, sizeof(data->htab1));
    memset(data->htab2, -1, sizeof(data->htab2));
    memset(data->htab3, -1, sizeof(data->htab3));
    memset(data->htab4, -1, sizeof(data->htab4));
    memset(data->htab5, -1, sizeof(data->htab5));
    memset(data->htab6, -1, sizeof(data->htab6));
    memset(data->htab7, -1, sizeof(data->htab7));
    memset(data->htab8, -1, sizeof(data->htab8));
#else
...
void compress(int fdin, int fdout, ThreadLocalData *data)
{
    ...
    data->bytes_out = 0; data->bytes_in = 0;
...
void decompress(int fdin, int fdout, ThreadLocalData *data)
{
    ...
    if (insize < 3 || data->inbuf[0] != MAGIC_1 || data->inbuf[1] != MAGIC_2)
    {
        ...
        if (insize > 0)
        {
            fprintf(stderr, "Thread: %s: not in compressed format\n",
                    data->ifname ? data->ifname : "stdin");
            data->exit_code = 1;
        }
    ...
    {
        fprintf(stderr, "Thread: %s: compressed with %d bits, can only handle %d bits\n",
                data->ifname ? data->ifname : "stdin", data->maxbits, BITS);
        data->exit_code = 4;
        return;
    }
...
resetbuf:	;
        ...
        while (inbits > posbits)
        {
            ...
            {
                if (code >= 256) {
                    fprintf(stderr, "Thread: corrupt input\n");
                    abort_compress(data);
                }
...
{
    fprintf(stderr, "Thread: read error on %s\n", 
            data->ifname ? data->ifname : "stdin");
    abort_compress(data);
}
...
{
    fprintf(stderr, "Thread: write error on %s\n", 
            data->ofname ? data->ofname : "stdout");
    abort_compress(data);
}
...
/* ===================== 文件处理函数 ===================== */
void comprexx(const char * fileptr, int file_id)
{
    int fdin = -1;
    int fdout = -1;
    int has_z_suffix;
    ...
    if (lstat(tempname, &local_data.infstat) == -1)
    {
        fprintf(stderr, "Thread %d: Cannot access %s\n", file_id, tempname);
        free(tempname);
        return;
    }

    if ((local_data.infstat.st_mode & S_IFMT) != S_IFREG)
    {
    ...
    if (local_data.do_decomp)
    {
        ...
        local_data.ofname = strdup(tempname);
        if (local_data.ofname == NULL)
        {
        ...
        if (has_z_suffix)
            local_data.ofname[namesize - 2] = '\0';
    }
    else
    {
        if (has_z_suffix)
        ...
        local_data.ofname = (char *)malloc(namesize + 3);
        if (local_data.ofname == NULL)
        {
            fprintf(stderr, "Thread %d: malloc error\n", file_id);
            free(tempname);
        ...
        memcpy(local_data.ofname, tempname, namesize);
        strcpy(&local_data.ofname[namesize], ".Z");
    }

    local_data.ifname = strdup(tempname);
    if (local_data.ifname == NULL)
    {
        fprintf(stderr, "Thread %d: strdup error\n", file_id);
        free(tempname);
    ...
    if (access(local_data.ofname, F_OK) == 0 && !local_data.force)
    {
        fprintf(stderr, "Thread %d: %s already exists\n", file_id, local_data.ofname);
        close(fdin);
        free(tempname);
        free_thread_local_data(&local_data);
        return;
    ...
    if ((fdout = open(local_data.ofname, O_WRONLY|O_CREAT|O_EXCL|O_BINARY, 0600)) == -1)
    {
    ...
    local_data.remove_ofname = 1;

    if (local_data.do_decomp == 0)
        compress(fdin, fdout, &local_data);
    else
        decompress(fdin, fdout, &local_data);
    ...
    if (!local_data.quiet)
    {
        if (local_data.do_decomp == 0)
        {
            printf("Thread %d: Compressed %s -> %s (%.2f%%)\n", 
                   file_id, tempname, local_data.ofname,
                   (local_data.bytes_in > 0) ? 
                   (100.0 * (local_data.bytes_in - local_data.bytes_out) / local_data.bytes_in) : 0.0);
        }
        else
        {
            printf("Thread %d: Decompressed %s -> %s\n", 
                   file_id, tempname, local_data.ofname);
        }
    }
    ...
    while (1) {
        ...
        if (got_task) {
            ...
            for (int n = 0; n < task.rounds; n++) {
                // 压缩模式
                comprexx(name, task.file_id);
                
                // 解压模式（在实际应用中，可能需要修改文件名）
                // 这里简化处理，假设文件已存在
            }
...
int main(int argc, char *argv[])
{
    ...
    // 检查参数
    if (argc < 3) {
        fprintf(stderr, "用法: %s <线程数> <配置文件名> [循环次数]\n", argv[0]);
        fprintf(stderr, "示例: %s 4 dataset.conf 5\n", argv[0]);
        fprintf(stderr, "\n参数说明:\n");
        fprintf(stderr, "  线程数: 并行处理的线程数量 (1-32)\n");
        fprintf(stderr, "  配置文件名: 包含数据文件列表的文本文件\n");
        fprintf(stderr, "  循环次数: 每个文件的处理轮数 (可选，默认1)\n");
        fprintf(stderr, "\n配置文件格式:\n");
        fprintf(stderr, "  每行一个文件编号，例如:\n");
        fprintf(stderr, "  1\n");
        fprintf(stderr, "  2\n");
        fprintf(stderr, "  3\n");
        fprintf(stderr, "  ...\n");
    ...
    if (num_threads <= 0 || num_threads > 256) {
        fprintf(stderr, "错误: 线程数必须在1-256之间\n");
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (sscanf(line, "%d", &file_id) == 1) {
            if (file_count < 1000) {
                ...
                fprintf(stderr, "警告: 文件数量超过最大值1000\n");
                break;
            }
        } else if (line_num == 1) {
            // 第一行可能是标题，忽略
            ...
            fprintf(stderr, "警告: 第%d行不是有效的数字: %s\n", line_num, line);
        }
    }
    ...
    if (file_count == 0) {
        fprintf(stderr, "\n错误: 配置文件中没有找到有效的文件编号\n");
        fprintf(stderr, "配置文件应该包含每行一个数字，例如:\n");
        fprintf(stderr, "1\n");
        fprintf(stderr, "2\n");
        fprintf(stderr, "3\n");
        return 1;
    ...
    if (file_count > 20) {
        printf("... 以及 %d 个更多文件", file_count - 20);
    }
    printf("\n");
    
    // 检查数据集目录是否存在
    struct stat st;
    if (stat("dataset", &st) != 0) {
        fprintf(stderr, "\n警告: dataset/ 目录不存在\n");
        fprintf(stderr, "请创建 dataset/ 目录并添加数据文件\n");
        fprintf(stderr, "例如: mkdir dataset\n");
        fprintf(stderr, "然后添加文件: dataset/1, dataset/2, 等等\n");
        return 1;
    }
    ...
    while (processed_files < total_files) {
        ...
        // 每处理10%报告一次进度
        int progress = (processed_files * 100) / total_files;
        if (progress >= last_reported + 10 || processed_files == total_files) {
            printf("进度: %d/%d 文件已处理 (%d%%)\n", 
                   processed_files, total_files, progress);
    ...
    printf("总时间: %lld 毫秒 (%.2f 秒)\n", total_time, total_time / 1000.0);
    printf("平均每文件时间: %.2f 毫秒\n", (float)total_time / total_files);
    printf("每秒操作数: %.2f\n", 
...
\n\
Original author version 4.0 (World release in 1985):\n\
